...
-   *Center* is usually interpreted as the most representative value. That's not always correct.
...
The median is the 2nd quartile, 0.5 quantile, and 50th percentile.

The `quantile()` function can return any quantile/percentile you desire. 
...
Correlation is unitless and ranges from -1 to 1. It expresses how close dots fall along a straight line. [See this useful Wikipedia illustration](https://en.wikipedia.org/wiki/Correlation_and_dependence#/media/File:Correlation_examples2.svg)
...
Most of the time we can only collect a sample (eg, voters, trees, cows). Hopefully it's a _random sample_. The mean of the sample data is _not_ the mean of the entire population. It is an _estimate_.
...
However it turns out the distribution of means calculated from most any distribution is pretty symmetric! This is the _Central Limit Theorem_. This means we can be fairly confident about standard error estimates.

```{r}
set.seed(1234)
means <- replicate(n = 1000, expr = {
  mean(rchisq(n = 30, df = 3))
})

op <- par(mfrow = c(1,2), pty = "s")
curve(dchisq(x, df = 3), from = 0, to = 10, ylab = "", xlab = "", 
      main = "skewed population\nmean = 3")
plot(density(means), ylab = "", xlab = "", main = "distribution of 1000 means\nfrom skewed population (n=30)")
par(op)
```
...
Historically p-values < 0.05 have been interpreted as "statistically significant", though many prominent statisticians are [now calling to abolish that threshold](https://www.tandfonline.com/doi/full/10.1080/00031305.2019.1583913).
...
It says each additional finished square foot adds \$111 to the expected value of a home. It also says a home with central air is expected to be worth almost \$50,000 than a home without central air. 

If we were interested in finsqft, we might say we have adjusted for the potential confounder cooling. 

**By the way, this may not be a good model!**
...
::: callout-tip
Controlled paranoia is useful for building and interpreting linear models.
::: 

## It's all linear models

Many statistical tests are special cases of a linear model.
