...
::: callout-tip
Data with no variability has a standard deviation/IQR of 0.
:::
...
op <- par(mfrow = c(1,2), pty = "s")
curve(dchisq(x, df = 8), from = 0, to = 20, ylab = "", xlab = "", 
...
**Example**: If painting cows like zebras has no effect on number of fly bites, we would expect to see the type of data we collected, or more extreme data, 0.0001 of the time.
...
We use our posterior distribution to calculate a _credible interval_. There's about 74% probability that the mean backpack weight is between 15.5 and 16.5 lbs. (Fig 3) 
