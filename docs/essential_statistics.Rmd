...
We'll also use functions in other packages occasionally. When we do we will access them using a double colon operator (::) instead of loading them with `library()`. For example, to use the `parse_number()` function from the {readr} package without loading it:

```{r}
x <- c("$2,345.23", "$2,232.98", "$345.87")
...
```{r}
URL <- "https://github.com/uvastatlab/phdplus2023/raw/main/data/homes.rds"
d <- readRDS(url(URL))
rm(URL)
```
...
Note: missing values are dropped by default! Set `addNA = TRUE` to see missing as a separate category
...
```{r}
# column 1; notice the result is transposed!
tab[,1]
```

Adding `drop=FALSE` preserves structure of table for viewing purposes.
...
```{r}
c(tab_p["Albemarle", "Yes"],
  tab_p["Monticello", "Yes"])
```
...
```{r}
tab_p["Albemarle", "Yes"] - tab_p["Monticello", "Yes"]
```
...
Portions of table can be extracted with indexing brackets; _need to use 3 dimensions_.
...
```{r}
# all rows, fp = "Yes", all strata 
tab2[, "Yes", ,drop = FALSE]
```


Calculate proportions across rows _within each strata_.
...
Can also use names of dimensions if you find that easier.
...
Let's look again at what proportion of homes have and do not have fireplaces within each hsdistrict, by cooling status. Notice that for homes with central air, Monticello has 0.317 without a fireplace versus 0.192 for Albemarle. 
...
What's the absolute and relative differences? First let's save the table.
...
```{r}
tab2_p["Monticello", "No", "Central Air"] - 
  tab2_p["Albemarle", "No", "Central Air"]
```


Of all homes with central air, the proportion of homes without a fireplace in Monticello is 0.125 higher than the proportion of homes with a fireplace in Albemarle.
...
Of all homes with central air, Monticello has about 65% more homes without a fireplace than Albemarle.

If you need a table as a data frame, which is needed for packages such as {ggplot2}, use the `as.data.frame()` function to make the conversion. By default the response name is labeled "Freq". You can change it with `responseName = "prop"`

```{r}
tab2_df <- as.data.frame(tab2_p, responseName = "prop")
head(tab2_df)
```

Now use with ggplot2
...
We can also calculate counts and proportions based on a condition.
...
These conditions can be included in a call to `xtabs()`; wrap in parentheses.
...
The tidyverse way for counts and proportions requires more work and returns data frames. Google for more information.
...
(2) What proportion of homes are on more than 1 acre of land (lotsize) within each hsdistrict?
...
In this case we may want to look at lotsizes less than 3 acres since the 3rd quartile is about 2.6. ggplot makes the pretty easy via the `xlim()` function
...
What quantile is a $1,000,000 home in?

```{r}
Fn(1e6)   
```

It's worth noting this is the same as finding the proportion of homes with total value less than or equal to one million.

```{r}
mean(d$totalvalue <= 1e6)
```

But a ECDF function allows us to calculate lots of quantiles at once.

```{r}
# From $100,000 to $1,000,000 in steps of $100,000
Fn(seq(1e5,1e6,1e5))
```
...
We often need to calculate summaries by groups. For example, what is the median totalvalue by high school district?
...
```{r}
# median totalvalue by hsdistrict
...
`tapply(numeric variable, grouping variable(s), function)`

```{r}
tapply(d$totalvalue, d$hsdistrict, median)
```

`tapply()` is ideal for use with functions that return _more than one value_. For example, using the `summary()` for totalvalue within each high school district.
...
For `tapply()`, wrap groups in `list()`. The result is a _matrix_.

```{r}
tapply(d$totalvalue, list(d$hsdistrict, d$cooling), median)
```


A matrix is often desirable for _reporting_.
A data frame is often desirable for _plotting_ and _modeling_. 

For example, below we use `aggregate()` to create a data frame for plotting.

```{r}
med_df <- aggregate(totalvalue ~ hsdistrict + cooling, data = d, median)
...
ggplot(med_df) +
  ...
  labs(title = 'Median home values by HS District and Central Air status',
       x = 'High School District', y = 'Total Home Value')
...
A theoretical estimate of the standard error of the sample mean is to take the standard deviation of the sample and divide by the square root of the sample size.
...
Our estimate of the population mean is our sample mean give or take the standard error. To be on the safe side, we usually give or take about two standard errors (ie, that's how we make a confidence interval).
...
Due to the Central Limit Theorem, the distribution of the sample means is fairly symmetric, so the standard deviation is a good measure of spread.
...
The above was for educational purposes. In practice, we don't get to take 10,000 random samples. Instead we would take our _single sample_ and estimate a 95% confidence interval using a function such as `smean.cl.normal()` from the {Hmisc} package.

```{r}
Hmisc::smean.cl.normal(samp)
```

R's built-in `t.test()` function can also calculate 95% confidence intervals.
...
(2) The variable "Pterygodermatites.dipodomis" indicates whether a sample was positive for the presence of Pterygodermatites dipodomis (1 = positive, 0 = negative). Calculate the proportion of rats with this parasite and a 95% confidence interval using the `prop.test()` function.
...
3. calculate probability of getting a statistic that big, or bigger (or that small, smaller) if null hypothesis is true
4. if the probability (the p-value) is small, then we have evidence against the null hypothesis

That's it. It's just a probability. p < 0.05 is not a threshold for certainty.

### Comparing two proportions

In the kangaroo rats data, about 35% had Pterygodermatites

```{r}
table(rats$Pterygodermatites.dipodomis) %>% proportions()
```

while about 20% had Mastophorus

```{r}
table(rats$Mastophorus.dipodomis) %>% proportions()

```

That's an absolute difference of about 15%.

Assume there is no difference in these proportions in the population. What's the probability we would see a difference as big or bigger than this if that's true.

We can use `prop.test()` to carry out this test. First we need to tally the number of "successes" and the number of "trials"

```{r}
cnt1 <- table(rats$Pterygodermatites.dipodomis)[2]
cnt2 <- table(rats$Mastophorus.dipodomis)[2]
n1 <- sum(table(rats$Pterygodermatites.dipodomis))
n2 <- sum(table(rats$Mastophorus.dipodomis))
```

Now plug these values into `prop.test()`

```{r}
prop.test(x = c(cnt1, cnt2), n = c(n1, n2))
```

The p-value is reported as 0.001786. This seems like good evidence that the null of no difference in proportions can be rejected.

The 95% confidence interval on the difference in proportions is [0.05, 0.23]. All of these values are equally plausible. 

### Comparing two means

Are male kangaroo rats heavier than female kangaroo rats on average? They seem slightly heavier.

```{r}
aggregate(Mass.g ~ Sex, data = rats, mean)
```

Assume there is no difference in the means. What's the probability we would see a difference as big or bigger than this if that's true?

We can use `t.test()` to carry out this test.

```{r}
t.test(Mass.g ~ Sex, data = rats)
```

The p-value is reported as 0.4048. It seems probable we would see data like we observed if there really is no difference in mass between the sexes. 

The 95% confidence interval on the difference in means overlaps 0 with both negative and positive numbers. All of these values are equally plausible. Whatever the "true" difference in means is between sexes, it's probably very small and not important.

It's good practice to visualize data when comparing means. Here's a simple box plot. We would like both groups to have similar variability and be roughly symmetric. Violations of those properties lead to other approaches such as log transformations, permutation tests, bootstrapping, and non-parametric tests.

```{r}
boxplot(Mass.g ~ Sex, data = rats)
```

Most statistical analyses require more sophisticated methods than simple hypothesis tests. 
...
We can attempt to model the totalvalue of a home using finsqft. In other words, create a math formula that allows us to estimate the expected totalvalue of a home based on its finsqft.
...
Let's use our model to make a prediction. For this we use the `predict()` function.
...
But is this a good model? Should we trust these predictions? A simple check is to look at the adjusted R-squared.

Adjusted R-squared:  0.578 
This suggests about 58% of the variation in totalvalue is "explained" by finsqft.

Can also look at _residuals_ versus _fitted values_ plot
...
Is this a good model? The residuals vs fitted values plot is a good first check.
...
Verdict: Now predicted values seem worse for smaller predicted values. But the model still seems like it may be useful

Let's visualize the model using the {ggeffects} package. The `ggpredict()` function makes predictions for the specified variable. We can pipe the result into `plot()` and see a fitted line through the scatter plot of totalvalue vs finsqft.

```{r}
ggpredict(m2, terms = "finsqft") %>% 
  plot(add.data = TRUE, dot.alpha = 0.1)
```

What's the expected value for a home with 2500 finsqft?

```{r}
predict(m2, newdata = data.frame(finsqft = 2500), interval = "confidence")
```

The result is on the log scale. Use `exp()` to return to the original scale.
...
### MODEL 3: two predictors with interaction

Let's build a more complex model with two predictors: finsqft and cooling. We will let them interact. That is, we believe the effect of finsqft depends on cooling status.

We define an interaction with a colon `:`. This says the association between totalvalue and finsqft depends on cooling.

```{r}
m3 <- lm(log(totalvalue) ~ finsqft + cooling + finsqft:cooling, 
         data = d)
summary(m3)
```

The last row of the summary output tests the interaction. It certainly seems to "significant" but is difficult to interpret.

This seems like an OK model, at least for most homes. The residuals versus fitted values plot seems reasonable for most homes.
...
The interaction appears to happen after 7500 finsqft. But the thick confidence ribbon indicates there are few (if any) homes that big without cooling.
...
```{r}
ggpredict(m3, terms = c("finsqft[1000:5000]", "cooling")) %>% 
  plot()
```

This interaction is small and not important.

In the model, notice we have a coefficient for "Central Air" only. That's because "No Central Air" is the reference level or baseline.

```{r}
coef(m3)
```

For homes with "No Central Air", the effect of finsqft is the finsqft coefficient. About a 5% increase for each additional 100 finsqft.

```{r}
coef(m3)[2] * 100
```

For homes with "Central Air", the effect of finsqft is the finsqft coefficient plus the interaction term. About a 4.5% increase for each additional 100 finsqft.

```{r}
sum(coef(m3)[2] + coef(m3)[4])* 100
...
Lots of material was cut for time, but it's included below if you're interested.


## Appendix: tidyverse grouped summaries

Some R users prefer using tidyverse functions for grouped summaries, but notice the missing values are included by default in the legend.
...
If we want to drop the NAs, we can add `scale_color_discrete(na.translate = FALSE)`
...
We can also filter them out using `!is.na()`. The means filter out the records that are NOT missing a value for cooling.
...
## Apendix: correlation for data frames


The `cor()` function works for data frames. This allows us to calculate correlation matrices. Below we select four columns and then pipe into the `cor()` function.  
...
## Appendix: Uncertainty about a proportion

Let's take a random sample of 50 homes' fireplace status. The fp variable is a categorical variable with levels "Yes" and "No". Below we form a vector of TRUE/FALSE and take the mean of TRUE, which returns the proportion of TRUE. We use `na.rm = TRUE` in the `mean()` function because some homes are missing this information in our data.

```{r}
samp2 <- sample(d$fp, 50)
mean(samp2 == "Yes", na.rm = TRUE)
```
...
```{r}
SE_p <- sd(props)
mean(samp2 == "Yes", na.rm = TRUE) + c(-1, 1)*2*SE_p
```
...
```{r}
prop.test(x = sum(samp2 == "Yes", na.rm = TRUE), n = 50)
prop.test(x = sum(samp2 == "Yes", na.rm = TRUE), n = 50)$conf.int
```
...
```{r}
binom::binom.confint(x = sum(samp2 == "Yes", na.rm = TRUE), n = 50)
```


## Appendix: 95% Confidence intervals

The _process_ of calculating a confidence interval usually works 95% of the time. The "95%" is due to the fact that about 95% of the probability under a normal distribution is within two standard errors of the mean. And most sampling distributions are approximately normal.
...
Let's sample 30 values from this population. We can do this with the `rnorm()` function.
...
Repeat the process 1000 times using `replicate()` (ie, sample and calculate 1000 confidence intervals):
...
```{r}
samp <- sample(d$totalvalue, 50)
tout <- t.test(samp, conf.level = 0.95)
...
The coverage is actually not 95%! (At least not with a sample size of 50.) This can happen when sampling from very skewed populations.
...
ci_test_log <- replicate(n = 1000, expr = {
  samp <- sample(d$totalvalue, 50, replace = TRUE)
  tout <- t.test(log(samp), conf.level = 0.95)  # Notice log() transform
...
When calculating standard error, we can do something called _bootstrapping_ that treats the sample like the population. This is helpful for smaller data sets and for statistics that don't have easy to calculate theoretical standard errors.
...
```{r}
# install the pbapply package so we can have a progress bar
...
The `ftable()` function can help display tables with 3 or more variables. The "f" means "flatten".

```{r}
tab2 <- xtabs(~ hsdistrict + fp + cooling, data = d)
ftable(tab2, exclude = NULL)
```

Specify row and column variables with the `row.vars` and `col.vars` arguments. 

```{r}
ftable(tab2, row.vars = c("cooling", "fp"), col.vars = "hsdistrict")
```


### Appendix: Chi-square test of association
...
### Appendix: two sample t-test with log transformation
...
Instead, if we exponentiate, we get a 95% CI for the _ratio of the geometric means_. 
...
The geometric mean of homes without fireplaces is about 44% - 43% lower than homes with fireplaces. (ie, \$277,741 is about 43-44% lower than \$491,747)
...
### Appendix: Wilcoxon Rank Sum test (aka Mann-Whitney)
...
The "difference in location" estimates _the median of the difference between a sample from homes with fireplaces and a sample from homes without fireplaces_.
...
## Appendix: modeling non-linear effects

In models `m1` and `m2` we assumed the effect of finsqft was _linear_ (ie, it had a constant slope). Going from 1500 - 1600 finsqft has the same effect on totalvalue as going from 2500 - 2600 finsqft.
...
One way to compare models is via information criteria such as AIC (Akaike information criterion). AIC is a statistic designed to help us choose a model with the best predictive power among a group of models. The value of  AIC doesn't really have any interpretation. It's meant to be compared to other AIC values. The lower the AIC the better.  "df" is short for "degrees of freedom" and is the number of parameters estimated in the model. This includes the residual standard error.

```{r}
AIC(m2, m3)
```

We can only compare models with the _same response_ fit to the same data. That's why we can't include the first model, m1. It was fit to the raw data. The next two models, m2 and m3, were fit to log-transformed data.
...
Expected mean value of _homes_ with 2500 finsqft
...
Expected value of _a home_ with 2500 finsqft
...
## Appendix: Example of Bayesian analysis

The {rstanarm} package provides a nice interface to the powerful Stan programming language for fitting Bayesian models. The formula syntax is the same as what `lm()` and `glm()` uses. The only difference is we need to specify _prior distributions_ for our parameters.

### Basic t-test type of analysis

There is no 'hypothesis testing' in Bayesian statistics. So to do something like a t-test, you instead you propose a prior distribution on the difference in means, and then get a posterior distribution after fitting a model. Recall above that a t-test is a special case of a linear model with a single binary predictor. We can use this insight to fit a Bayesian model for totalvalue as a function cooling.

We use the `prior_*` arguments to set the prior distributions. Notice we have to set separate priors for the intercept and "aux" parameters. The "aux" parameter is the "residual standard error". The `prior` argument is for the slope parameter in the model, which corresponds to the difference in means. 

The `normal()` and `exponential()` functions are provided by the rstanarm package. `location` refers to "mean" and `scale` refers to "standard deviation". I set the location to 0 and the scale to 1e6 in the `normal` priors. These are "weakly informative", which is to say they basically rule out implausible values. Notice the model takes several seconds to run. It uses a sampling algorithm instead of a closed-form mathematical formula.

```{r}
install.packages("rstanarm")
library(rstanarm)
bm1 <- stan_glm(totalvalue ~ cooling, data = d, 
                prior_intercept = normal(location = 0, scale = 1e6),
                prior_aux = exponential(rate = 1),
                prior = normal(location = 0, scale = 1e6))
```
Use the `summary()` function to see the model results. The main result is three _posterior distributions_ for the parameters. There are no test statistics or p-values.
...
Use the `posterior_interval()` function to get 95% credible intervals on the parameters. When I ran this I got a result that said there's about 0.95 probability that the difference in totalvalue between homes with and without cooling is between 222,824 and 232,474.

```{r}
posterior_interval(bm1)
```

See the vignettes that accomplany the {rstanarm} package for more details and examples of Bayesian data analysis.


## Appendix: Many hypothesis tests are special cases of linear models

### ANOVA (one categorical predictor)

We can carry out ANOVA using `aov()`...

```{r}
aov1 <- aov(totalvalue ~ hsdistrict, data = d)
summary(aov1)
```

Or fit a linear model using `lm()`. (The `aov()` function uses `lm()` under the hood.)
...
We can do a t-test with `t.test()`...

```{r}
t.test(log(totalvalue) ~ cooling, data = d, var.equal = TRUE)
```

Or fit a linear model using `lm()`
...
We can compare two proportions using `prop.test()`...

```{r}
xtabs(~ fp + remodel, data = d) %>% 
  addmargins()
```

```{r}
prop.test(x = c(21740, 1948), n = c(29870, 2771), correct = FALSE)
```

Or fit a logistic regression model using `glm()`

```{r}
m7 <- glm(fp ~ remodel, data = d, family = binomial)
summary(m7)
```

```{r}
# proportions
predict(m7, newdata = data.frame(remodel = c(0,1)), 
        type = "response")
```
