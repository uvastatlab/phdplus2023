...
We can also use `xtabs()` function that allows us to use R's formula syntax. The synatx `~ hsdistrict` means count up by hsdistrict. Notice it includes the name of the variable in the output.
...
Cross-tabulation of hsdistrict and fp (fireplace status) _stratified_ by cooling status: `~ hsdistrict + fp + cooling` (The last variable will be the stratifying variable, or 3rd dimension)
...
`summary()` and `hist()` make a powerful combination for summarizing one variable. Let's investidate the lotsize variable.
