...
`tapply()` is ideal for use with functions that return more than one value. For example, using the `summary()` for totalvalue within each high school district.
...
ci_test_log <- replicate(n = 1000, expr = {
  samp <- sample(d$totalvalue, 50, replace = TRUE)
  tout <- t.test(log(samp), conf.level = 0.95)
...
```{r}
rats <- read.csv("../data/parasite.csv")
```
