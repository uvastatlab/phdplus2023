...
Let's look again at what proportion of homes have and do not have fireplaces within each hsidstrict, by cooling status. Notice that for homes with central air, Monticello has 0.32 without a fireplace versus 0.19 for Albemarle. 
...
We often need to calculate summaries by groups. For example, what is the mean totalvalue by high school district?
...
The above was for educational purposes. In practice, we don't get to take 10,000 random samples. Instead we would take our _single sample_ and estimate a confidence interval using a math formula courtesy of a function such as `t.test()`
...
That's it. It's just a probability. p < 0.05 is not a magic threshold for certainty.
...
The p-value is reported as 0.001786. This seems like good evidence that the proportion of rats with Pterygodermatites is higher. 
...
Let's visualize the model using the {ggeffects} package. The `ggpredict()` function makes predictions for the specified variable. We can pipe the result in `plot()` and see a fitted line through the scatter plot of totalvalue vs finsqft.
